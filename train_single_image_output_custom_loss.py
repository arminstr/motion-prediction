...
def only_bright_pixels_custom(y_true, y_pred):
    ...
    mask = tf.math.greater(y_true, threshold)
    print("mask:", mask)
    ...
    print("y_pred_masked:", y_pred_masked)
...
def load_scenarios(n_samples, path_name):
    ...
    label_frames = np.zeros((n_samples, GRID_SIZE, GRID_SIZE, 1), dtype=np.float32)
    ...
    for directory in directories:
        ...
        for filename in filenames:
            image = cv2.imread(path_name + '/' + directory + '/' + filename)
